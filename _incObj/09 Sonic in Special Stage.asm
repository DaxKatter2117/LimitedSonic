...
Obj09_Move:
		...
		bsr.w	Obj09_MoveLeft			;	branch to left movement subroutine
		
Obj09_ChkRight:
		btst	#bitR,(v_jpadhold2).w	;	is right being pressed
		beq.s	Obj09_MoveSpeed			;	if not, branch
		bsr.w	Obj09_MoveRight			;	branch to right movement subroutine
...
Obj09_MoveCollision:		;	CODE TO CHECK STAGE COLLISION ON X-AXIS
		move.b	(v_ssangle).w,d0		;	check the Special Stage Angle
		addi.b	#$20,d0					;	?
		andi.b	#$C0,d0					;	
		neg.b	d0						;	?
		jsr	(CalcSine).l				;	load speed trajectory algorithms
		muls.w	obVelX(a0),d1			;	calculate x-speed trajectory
		add.l	d1,obX(a0)				;	calculate Sonic's next x position
		muls.w	obVelY(a0),d0			;	calculate y-speed trajectory
		add.l	d0,obY(a0)				;	calculate Sonic's next y position
		movem.l	d0-d1,-(sp)				;	save trajectory values
		move.l	obY(a0),d2				;	move sonic's next y-position into d2
		move.l	obX(a0),d3				;	move sonic's next x-position into d3
		bsr.w	sub_1BCE8				;	branch to the collision detection routine
		beq.s	loc_1BAF2				;	if Sonic's not colliding with anything branch
		movem.l	(sp)+,d0-d1				;	remove x and y speed trajectory from the stack
		sub.l	d1,obX(a0)				;	cancel out Sonic's next x-position
		sub.l	d0,obY(a0)				;	cancel out Sonic's next y-position
		move.w	#0,obVelX				;	Make Sonic Stop
		rts								;	end routine

loc_1BAF2:		;	CODE TO RUN IF NO COLLISION DETECTED
		movem.l	(sp)+,d0-d1				;	remove x and y speed trajectory from the stack
		rts	

Obj09_MoveLeft:
		bset	#0,obStatus(a0)			;	set Sonic to display flipped sprites
		move.w	#-400,obVelX(a0)		;	Make Sonic's X-speed -$300
		rts								;	end routine
...
Obj09_Rise:
		btst	#bitUp,(v_jpadhold2).w	;	is left being pressed
		beq.s	Obj09_NoRise			;	if not, branch
		bsr.w	Obj09_MoveUp
		
Obj09_NoRise:
		btst	#bitDn,(v_jpadhold2).w	;	is Dn Pressed?
...
Obj09_MoveYCollision:		;	CODE TO CHECK STAGE COLLISION ON X-AXIS
		...
		sub.l	d1,obX(a0)				;	cancel out Sonic's next x-position
		sub.l	d0,obY(a0)				;	cancel out Sonic's next y-position
		move.w	#0,obVelY				;	Make Sonic Stop
		rts								;	end routine

Obj09_MoveUp:
		move.w	#-$300,obVelY(a0)		;	Make Sonic's X-speed -$300
		rts
...
Obj09_ExitStage:
		addi.w	#$40,(v_unused2).w
		cmpi.w	#$1800,(v_unused2).w
		bne.s	loc_1BBF4
		move.b	#id_Level,(v_gamemode).w

loc_1BBF4:
		cmpi.w	#$3000,(v_unused2).w
		blt.s	loc_1BC12
