Mus83_MZ_Header:
	smpsHeaderStartSong 1
	smpsHeaderVoice     Mus83_MZ_Voices
	smpsHeaderChan      $06, $03
	smpsHeaderTempo     $02, $09

	smpsHeaderDAC       Mus83_MZ_DAC
	smpsHeaderFM        Mus83_MZ_FM1,	$E8, $15
	smpsHeaderFM        Mus83_MZ_FM2,	$E8, $0E
	smpsHeaderFM        Mus83_MZ_FM3,	$E8, $15
	smpsHeaderFM        Mus83_MZ_FM4,	$E8, $17
	smpsHeaderFM        Mus83_MZ_FM5,	$E8, $17
	smpsHeaderPSG       Mus83_MZ_PSG1,	$D0, $03, $00, fTone_08
	smpsHeaderPSG       Mus83_MZ_PSG2,	$D0, $05, $00, fTone_08
	smpsHeaderPSG       Mus83_MZ_PSG3,	$0B, $03, $00, fTone_09

; FM3 Data
Mus83_MZ_FM3:
	smpsAlterNote       $02

; FM1 Data
Mus83_MZ_FM1:
	smpsSetvoice        $00
	dc.b	nRst, $24

Mus83_MZ_Jump04:
	smpsCall            Mus83_MZ_Call04
	dc.b	nA6, $09, nRst, $03, nA6, $06, nA6, nA6, $09, nRst, $03, nA6
	dc.b	$06, nA6, nA6, $09, nRst, $03, nA6, $06, nA6, nA6, $0C, nA6
	dc.b	nA6, $12, nA6, $35, nRst, $01
	smpsCall            Mus83_MZ_Call04
	dc.b	nA6, $24, nA6, $0C, nA6, $24, nA6, $09, nRst, $03, nA6, $12
	dc.b	nA6, $4D, nRst, $61, nRst, $48
	smpsJump            Mus83_MZ_Jump04

Mus83_MZ_Call04:
	dc.b	nA5, $06, nA5, nA6, nA6, nA6, $09, nRst, $03, nA6, $06, nA6
	dc.b	nA6, $09, nRst, $03, nA6, $06, nA6, nA6, $09, nRst, $03, nA6
	dc.b	$06, nA6, nA6, nA6, nA6, nA6, nA6, $0C, nA6, $06, smpsNoAttack, nA6
	dc.b	$4D, nRst, $01
	smpsReturn

; FM4 Data
Mus83_MZ_FM4:
	smpsSetvoice        $03
	smpsAlterVol        $F7
	dc.b	nRst, $06, nE5, $03, $03, $06, nRst, nE4, $1E
	smpsSetvoice        $03
	smpsAlterVol        $09
	dc.b	nE6, $06

Mus83_MZ_Jump03:
	smpsCall            Mus83_MZ_Call03
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst
	dc.b	$06, nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C
	dc.b	nRst, $18, nE6, $03, nRst, $0F, nE6, $03, nRst, $39, nE6, $06
	smpsCall            Mus83_MZ_Call03
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst
	dc.b	$06, nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C
	dc.b	nRst, $18, nE7, $03, nRst, $0F, nE7, $03, nRst, $09, nE7, $09
	dc.b	nRst, $03, nE7, nRst, nE7, $06, nRst, nE7, $03, nRst, nE6, $12
	smpsCall            Mus83_MZ_Call02
	smpsJump            Mus83_MZ_Jump03

Mus83_MZ_Call03:
	dc.b	smpsNoAttack, $03, nRst, nE6, nRst, nE7, $06, nRst, nE6, $0C, nRst, $06
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE7, $06, nRst, nE6, $0C, nRst
	dc.b	$18, nE7, $03, nRst, $0F, nE7, $03, nRst, $1B, nE7, $03, nRst
	dc.b	$0F, nE7, $03, nRst, $09
	smpsReturn

; FM5 Data
Mus83_MZ_FM5:
	smpsSetvoice        $04
	smpsAlterVol        $FC
	smpsAlterPitch      $24
	dc.b	nRst, $06, nE4, $03, $03, $06, nRst, nE3, $1E
	smpsSetvoice        $04
	smpsAlterPitch      $DC
	smpsAlterVol        $04
	dc.b	nE6, $06

Mus83_MZ_Jump02:
	smpsCall            Mus83_MZ_Call01
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst
	dc.b	$06, nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C
	dc.b	nRst, $18, nE6, $03, nRst, $0F, nE6, $03, nRst, $39, nE6, $06
	smpsCall            Mus83_MZ_Call01
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst
	dc.b	$06, nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C
	dc.b	nRst, $18, nE6, $03, nRst, $0F, nE6, $03, nRst, $09, nE7, $09
	dc.b	nRst, $03, nE7, nRst, nE6, $06, nRst, nE6, $03, nRst, nE6, $12
	smpsAlterNote       $03
	smpsCall            Mus83_MZ_Call02
	smpsAlterNote       $00
	smpsJump            Mus83_MZ_Jump02

Mus83_MZ_Call01:
	dc.b	smpsNoAttack, $03, nRst, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst, $06
	dc.b	nE6, $09, nRst, $03, nE6, nRst, nE6, $06, nRst, nE6, $0C, nRst
	dc.b	$18, nE6, $03, nRst, $0F, nE6, $03, nRst, $1B, nE6, $03, nRst
	dc.b	$0F, nE6, $03, nRst, $09
	smpsReturn

; FM2 Data
Mus83_MZ_FM2:
	smpsSetvoice        $01
	dc.b	nRst, $06, nE4, $03, nE4
	smpsNop             $01
	dc.b	nE4, $06, nRst, nE3, $24

Mus83_MZ_Jump01:
	smpsCall            Mus83_MZ_Call00

Mus83_MZ_Loop00:
	dc.b	nE3, $03, nRst, nE3, $06, nE4, $03, nRst, nE4, $06, nE3, $03
	dc.b	nRst, nE3, $06, nE4, $03, nRst, nE4, $06
	smpsLoop            $01, $02, Mus83_MZ_Loop00
	dc.b	nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE4, $03
	dc.b	nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE3, $03, nRst, nE3
	dc.b	$06, nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE3
	dc.b	$03, nRst, nE3, $06
	smpsCall            Mus83_MZ_Call00
	dc.b	nE3, $03, nRst, nE3, $06, nE4, $03, nRst, nE4, $06, nE4, $03
	dc.b	nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4
	dc.b	$06, nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE4
	dc.b	$03, nRst, nE4, $06, nE3, $03, nRst, nE3, $06, nE4, $03, nRst
	dc.b	nE4, $06, nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4, $06
	dc.b	nE3, $03, nRst, nE3, $06, nE4, $03, nRst, nE4, $06, nE4, $03
	dc.b	nRst, nE4, $06, nE4, $03, nRst, nE4, $06

Mus83_MZ_Loop01:
	dc.b	nE3, $12, nE3, $06, nE3, $12, nE3, $06, nE3, $12, nE3, $06
	dc.b	nE3, $12, nE3, $06
	smpsLoop            $01, $02, Mus83_MZ_Loop01
	smpsNop             $01
	smpsJump            Mus83_MZ_Jump01

Mus83_MZ_Call00:
	dc.b	nE3, $03, nRst, nE3, $06, nE4, $03, nRst, nE4, $06, nE4, $03
	dc.b	nRst, nE4, $06, nE4, $03, nRst, nE4, $06
	smpsLoop            $00, $02, Mus83_MZ_Call00

Mus83_MZ_Loop05:
	dc.b	nE4, $03, nRst, nE4, $06, nE4, $03, nRst, nE4, $06, nE4, $03
	dc.b	nRst, nE4, $06, nE4, $03, nRst, nE4, $06
	smpsLoop            $00, $02, Mus83_MZ_Loop05
	smpsReturn

; PSG1 Data
Mus83_MZ_PSG1:
	dc.b	nRst, $3C

Mus83_MZ_Jump06:
	dc.b	nRst, $60
	smpsCall            Mus83_MZ_Call06
	dc.b	nRst, $2A, nF7, $0C, nF7, $06, nF7, $0C, nF6, $06, nF6, $2A
	dc.b	nRst, $48
	smpsCall            Mus83_MZ_Call06
	dc.b	nRst, $60

Mus83_MZ_Loop03:
	dc.b	nF6, $06, nF7, $03, nF6, nF7, $06, nF6, nF6, nF6, nF6, nF6
	dc.b	nF6, nF6, $03, nF6, nF6, $06, nF6, nF6, nF6, nF6, nF6
	smpsLoop            $00, $02, Mus83_MZ_Loop03
	smpsJump            Mus83_MZ_Jump06

Mus83_MZ_Call06:
	dc.b	nRst, $30, nF7, $03, nF7, nF6, nF6, nF7, nF6, nF6, nF6, nF6
	dc.b	nF6, nF6, nF5, nF6, nF6, nF5, nF5, $27, nRst, $3C
	smpsReturn

; PSG2 Data
Mus83_MZ_PSG2:
	dc.b	nRst, $02
	smpsAlterNote       $01
	smpsJump            Mus83_MZ_PSG1

; PSG3 Data
Mus83_MZ_PSG3:
	smpsPSGform         $E7
	smpsPSGAlterVol     $FF
    if FixMusicAndSFXDataBugs
	dc.b	nRst, $06, nE4, $03, $03, $06, nRst, nE3, $24
    else
	; These first three notes are too high when combined with this track's
	; transposition value, causing them to overflow the PSG frequency table
	; and play invalid notes. In the Sonic 1 prototype, this problem was
	; even worse as there were smpsChangeTransposition commands around the
	; following line that raised the notes yet another octave higher,
	; causing the fourth note to break too.
	; Since the commands are gone now, we can assume that this was the
	; developers' intended solution, though it unfortunately only fixed the
	; fourth note. So, in order to fix the bug for good, the notes on the
	; following line have to be lowered by yet another octave.
	dc.b	nRst, $06, nE5, $03, $03, $06, nRst, nE4, $24
    endif
	smpsPSGAlterVol     $01

Mus83_MZ_Jump05:
	smpsCall            Mus83_MZ_Call05
	dc.b	nE3, nE3, nE4, nE4, nE3, nE3, nE4, nE4, nE3, nE3, nE4, nE4
	dc.b	nE3, nE3, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4
	dc.b	nE3, nE3, nE4, nE4, nE4, nE4, nE3, nE3
	smpsCall            Mus83_MZ_Call05
	dc.b	nE3, nE3, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4
	dc.b	nE4, nE4, nE4, nE4, nE3, nE3, nE4, nE4, nE4, nE4, nE4, nE4
	dc.b	nE3, nE3, nE4, nE4, nE4, nE4, nE4, nE4
	smpsPSGAlterVol     $FF

Mus83_MZ_Loop02:
	dc.b	nE4, $12, nE4, $06, nE4, $12, nE4, $06, nE4, $12, nE4, $06
	dc.b	nE4, $12, nE4, $06
	smpsLoop            $00, $02, Mus83_MZ_Loop02
	smpsPSGAlterVol     $01
	smpsJump            Mus83_MZ_Jump05

Mus83_MZ_Call05:
	dc.b	nE3, $06, nE3, nE4, nE4, nE4, nE4, nE4, nE4, nE3, nE3, nE4
	dc.b	nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4
	dc.b	nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4, nE4
	smpsReturn

; DAC Data
Mus83_MZ_DAC:
	dc.b	nRst, $06, dSnare, $03, $03, $0C, dSnare, $0C, $0C, $0C

Mus83_MZ_Jump00:
	dc.b	dSnare, $0C
	smpsJump            Mus83_MZ_Jump00

Mus83_MZ_Call02:
	smpsNoteFill        $06

Mus83_MZ_Loop04:
	dc.b	nRst, $06, nE7, nE7, nE6, $0C, nE7, $06, nE6, nE6, nRst, nE7
	dc.b	nE6, nE6, $0C, nE7, $06, nE6, nE6
	smpsLoop            $00, $02, Mus83_MZ_Loop04
	smpsNoteFill        $00
	smpsReturn

Mus83_MZ_Voices:
;	Voice $00
;	$22
;	$0A, $13, $05, $11, 	$03, $12, $12, $11, 	$00, $13, $13, $00
;	$03, $02, $02, $01, 	$1F, $1F, $0F, $0F, 	$1E, $18, $26, $81
	smpsVcAlgorithm     $02
	smpsVcFeedback      $04
	smpsVcUnusedBits    $00
	smpsVcDetune        $01, $00, $01, $00
	smpsVcCoarseFreq    $01, $05, $03, $0A
	smpsVcRateScale     $00, $00, $00, $00
	smpsVcAttackRate    $11, $12, $12, $03
	smpsVcAmpMod        $00, $00, $00, $00
	smpsVcDecayRate1    $00, $13, $13, $00
	smpsVcDecayRate2    $01, $02, $02, $03
	smpsVcDecayLevel    $00, $00, $01, $01
	smpsVcReleaseRate   $0F, $0F, $0F, $0F
	smpsVcTotalLevel    $01, $26, $18, $1E

;	Voice $01
;	$3A
;	$61, $3C, $14, $31, 	$9C, $DB, $9C, $DA, 	$04, $09, $04, $03
;	$03, $01, $03, $00, 	$1F, $0F, $0F, $AF, 	$21, $47, $31, $80
	smpsVcAlgorithm     $02
	smpsVcFeedback      $07
	smpsVcUnusedBits    $00
	smpsVcDetune        $03, $01, $03, $06
	smpsVcCoarseFreq    $01, $04, $0C, $01
	smpsVcRateScale     $03, $02, $03, $02
	smpsVcAttackRate    $1A, $1C, $1B, $1C
	smpsVcAmpMod        $00, $00, $00, $00
	smpsVcDecayRate1    $03, $04, $09, $04
	smpsVcDecayRate2    $00, $03, $01, $03
	smpsVcDecayLevel    $0A, $00, $00, $01
	smpsVcReleaseRate   $0F, $0F, $0F, $0F
	smpsVcTotalLevel    $00, $31, $47, $21

;	Voice $02
;	$3A
;	$01, $07, $01, $01, 	$8E, $8E, $8D, $53, 	$0E, $0E, $0E, $03
;	$00, $00, $00, $00, 	$1F, $FF, $1F, $0F, 	$18, $28, $27, $80
	smpsVcAlgorithm     $02
	smpsVcFeedback      $07
	smpsVcUnusedBits    $00
	smpsVcDetune        $00, $00, $00, $00
	smpsVcCoarseFreq    $01, $01, $07, $01
	smpsVcRateScale     $01, $02, $02, $02
	smpsVcAttackRate    $13, $0D, $0E, $0E
	smpsVcAmpMod        $00, $00, $00, $00
	smpsVcDecayRate1    $03, $0E, $0E, $0E
	smpsVcDecayRate2    $00, $00, $00, $00
	smpsVcDecayLevel    $00, $01, $0F, $01
	smpsVcReleaseRate   $0F, $0F, $0F, $0F
	smpsVcTotalLevel    $00, $27, $28, $18

;	Voice $03
;	$23
;	$7C, $32, $00, $00, 	$5F, $58, $DC, $DF, 	$04, $0B, $04, $04
;	$06, $0C, $08, $08, 	$1F, $1F, $BF, $BF, 	$24, $26, $16, $80
	smpsVcAlgorithm     $03
	smpsVcFeedback      $04
	smpsVcUnusedBits    $00
	smpsVcDetune        $00, $00, $03, $07
	smpsVcCoarseFreq    $00, $00, $02, $0C
	smpsVcRateScale     $03, $03, $01, $01
	smpsVcAttackRate    $1F, $1C, $18, $1F
	smpsVcAmpMod        $00, $00, $00, $00
	smpsVcDecayRate1    $04, $04, $0B, $04
	smpsVcDecayRate2    $08, $08, $0C, $06
	smpsVcDecayLevel    $0B, $0B, $01, $01
	smpsVcReleaseRate   $0F, $0F, $0F, $0F
	smpsVcTotalLevel    $00, $16, $26, $24

;	Voice $04
;	$02
;	$3C, $32, $55, $51, 	$1F, $98, $1F, $9F, 	$0F, $11, $0E, $11
;	$0E, $05, $08, $05, 	$5F, $0F, $6F, $0F, 	$2D, $2D, $2F, $80
	smpsVcAlgorithm     $02
	smpsVcFeedback      $00
	smpsVcUnusedBits    $00
	smpsVcDetune        $05, $05, $03, $03
	smpsVcCoarseFreq    $01, $05, $02, $0C
	smpsVcRateScale     $02, $00, $02, $00
	smpsVcAttackRate    $1F, $1F, $18, $1F
	smpsVcAmpMod        $00, $00, $00, $00
	smpsVcDecayRate1    $11, $0E, $11, $0F
	smpsVcDecayRate2    $05, $08, $05, $0E
	smpsVcDecayLevel    $00, $06, $00, $05
	smpsVcReleaseRate   $0F, $0F, $0F, $0F
	smpsVcTotalLevel    $00, $2F, $2D, $2D

