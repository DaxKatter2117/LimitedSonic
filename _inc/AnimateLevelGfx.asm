...
.size		= 8	; number of tiles per frame
		...
		andi.w	#0,d0		; there are only 2 frames
...
.size		= 12	; number of tiles per frame
		...
		andi.w	#0,d0		; there are 4 frames
...
.size		= 8	; number of tiles per frame
		...
		andi.b	#0,d0		; there are 3 frames
...
AniArt_MZ_Magma:
		...
		move.b	(v_oscillate+$0).w,d3 ; get oscillating value
		move.w	#3,d2
...
.size		= 6	; number of tiles per frame
		...
		andi.b	#0,(v_lani3_frame).w ; there are 3 frames
		mulu.w	#.size*$20,d0
		adda.w	d0,a1		; jump to appropriate tile
